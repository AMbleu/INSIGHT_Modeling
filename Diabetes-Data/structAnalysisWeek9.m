...
for code = codeNums
   ...
   tCode{iCode} = tmin(iMeas); 
   val{iCode} = dataForSubj(iMeas); 
   iCode = iCode+1;
end
...
subplot(7,1,i);
iCode = iCodes(i);
plot(tCode{iCode}, val{iCode});
axis([minTime maxTime -inf inf]);
ylabel(num2str(codeNums(iCode)));
end
...
%    figure(codeNums2); 
%    hold on
%    plot(tmin(iMeas), dataForSubj(iMeas), 'r'); 
%    x2 = tmin(iMeas); 
